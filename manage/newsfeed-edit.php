...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE jp_news SET news_title=%s, news_desc=%s, news_date=%s, news_status=%s WHERE news_id=%s",
  ...
  $Result1 = mysql_query($updateSQL, $conPerak) or die(mysql_error());

  $updateGoTo = "newsfeed.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsNewsEdit = "-1";
if (isset($_GET['news_id'])) {
  $colname_rsNewsEdit = $_GET['news_id'];
}
mysql_select_db($database_conPerak, $conPerak);
$query_rsNewsEdit = sprintf("SELECT * FROM jp_news WHERE news_id = %s", GetSQLValueString($colname_rsNewsEdit, "int"));
$rsNewsEdit = mysql_query($query_rsNewsEdit, $conPerak) or die(mysql_error());
$row_rsNewsEdit = mysql_fetch_assoc($rsNewsEdit);
$totalRows_rsNewsEdit = mysql_num_rows($rsNewsEdit);
        ...
        <form action="<?php echo $editFormAction; ?>" method="POST" name="form1">
          <table align="center" class="table table-striped">
            <tr valign="baseline">
              ...
              <td><input type="text" name="news_title" value="<?php echo $row_rsNewsEdit['news_title']; ?>" size="32"></td>
            </tr>
            <tr valign="baseline">
              <td nowrap align="right" valign="top">Description</td>
              <td><textarea name="news_desc" cols="500" rows="10" style="width:500px"><?php echo $row_rsNewsEdit['news_desc']; ?></textarea></td>
            </tr>
            <tr valign="baseline">
              <td nowrap align="right">Date</td>
              <td><input type="text" name="news_date" value="<?php echo $row_rsNewsEdit['news_date']; ?>" size="32"> <em>ex: 27-11-2012</em></td>
            </tr>
            <tr valign="baseline">
              <td nowrap align="right">Status</td>
              <td>
                <select name="news_status">
                  <option value="pending" <?php if (!(strcmp("pending", $row_rsNewsEdit['news_status']))) {echo "selected=\"selected\"";} ?>>Pending</option>
                  <option value="published" <?php if (!(strcmp("published", $row_rsNewsEdit['news_status']))) {echo "selected=\"selected\"";} ?>>Publish</option>
                  <option value="draft" <?php if (!(strcmp("draft", $row_rsNewsEdit['news_status']))) {echo "selected=\"selected\"";} ?>>Draft</option>
                </select>
              </td>
            </tr>
            <tr valign="baseline">
              <td nowrap align="right">&nbsp;</td>
              <td><input type="submit" value="Update News" class="btn btn-success"></td>
            </tr>
          </table>
          <input type="hidden" name="news_id" value="<?php echo $row_rsNewsEdit['news_id']; ?>">
