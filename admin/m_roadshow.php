...
$maxRows_rsModuleRoadshow = 50;
$pageNum_rsModuleRoadshow = 0;
if (isset($_GET['pageNum_rsModuleRoadshow'])) {
  $pageNum_rsModuleRoadshow = $_GET['pageNum_rsModuleRoadshow'];
}
$startRow_rsModuleRoadshow = $pageNum_rsModuleRoadshow * $maxRows_rsModuleRoadshow;

mysql_select_db($database_conJobsPerak, $conJobsPerak);
$query_rsModuleRoadshow = "SELECT * FROM jp_roadshow";
$query_limit_rsModuleRoadshow = sprintf("%s LIMIT %d, %d", $query_rsModuleRoadshow, $startRow_rsModuleRoadshow, $maxRows_rsModuleRoadshow);
$rsModuleRoadshow = mysql_query($query_limit_rsModuleRoadshow, $conJobsPerak) or die(mysql_error());
$row_rsModuleRoadshow = mysql_fetch_assoc($rsModuleRoadshow);

if (isset($_GET['totalRows_rsModuleRoadshow'])) {
  $totalRows_rsModuleRoadshow = $_GET['totalRows_rsModuleRoadshow'];
} else {
  $all_rsModuleRoadshow = mysql_query($query_rsModuleRoadshow);
  $totalRows_rsModuleRoadshow = mysql_num_rows($all_rsModuleRoadshow);
}
$totalPages_rsModuleRoadshow = ceil($totalRows_rsModuleRoadshow/$maxRows_rsModuleRoadshow)-1;

$queryString_rsModuleRoadshow = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rsModuleRoadshow") == false && 
        stristr($param, "totalRows_rsModuleRoadshow") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rsModuleRoadshow = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rsModuleRoadshow = sprintf("&totalRows_rsModuleRoadshow=%d%s", $totalRows_rsModuleRoadshow, $queryString_rsModuleRoadshow);
        ...
        <table border="0">
          <tr>
            <td><?php if ($pageNum_rsModuleRoadshow > 0) { // Show if not first page ?>
                <a href="<?php printf("%s?pageNum_rsModuleRoadshow=%d%s", $currentPage, 0, $queryString_rsModuleRoadshow); ?>">First</a>
                <?php } // Show if not first page ?></td>
            <td><?php if ($pageNum_rsModuleRoadshow > 0) { // Show if not first page ?>
                <a href="<?php printf("%s?pageNum_rsModuleRoadshow=%d%s", $currentPage, max(0, $pageNum_rsModuleRoadshow - 1), $queryString_rsModuleRoadshow); ?>">Previous</a>
                <?php } // Show if not first page ?></td>
            <td><?php if ($pageNum_rsModuleRoadshow < $totalPages_rsModuleRoadshow) { // Show if not last page ?>
                <a href="<?php printf("%s?pageNum_rsModuleRoadshow=%d%s", $currentPage, min($totalPages_rsModuleRoadshow, $pageNum_rsModuleRoadshow + 1), $queryString_rsModuleRoadshow); ?>">Next</a>
                <?php } // Show if not last page ?></td>
            <td><?php if ($pageNum_rsModuleRoadshow < $totalPages_rsModuleRoadshow) { // Show if not last page ?>
                <a href="<?php printf("%s?pageNum_rsModuleRoadshow=%d%s", $currentPage, $totalPages_rsModuleRoadshow, $queryString_rsModuleRoadshow); ?>">Last</a>
                <?php } // Show if not last page ?></td>
          </tr>
        </table>
Records <?php echo ($startRow_rsModuleRoadshow + 1) ?> to <?php echo min($startRow_rsModuleRoadshow + $maxRows_rsModuleRoadshow, $totalRows_rsModuleRoadshow) ?> of <?php echo $totalRows_rsModuleRoadshow ?> </div><!-- end of right content-->
...
mysql_free_result($rsModuleRoadshow);
